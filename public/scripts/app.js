...
function () {
    angular.module("app.ui.form.directives", []).directive("uiRangeSlider", [
        ...
        angular.module("app.ui.directives", []).directive("uiTime", [
            ...
            "use strict";
            angular.module("app", ["ngRoute", "ngAnimate", "ui.select", "ui.bootstrap", "easypiechart", "mgo-angular-wizard", "textAngular", "app.ui.directives", "app.controllers", "app.directives", "app.ui.form.directives", "app.chart.directives", "timer", "socket-io"]).config(["$routeProvider", function ($routeProvider) {
                ...
                }).when("/game/boxscore/:game_id", {
                    templateUrl: "views/pages/game-boxscore.html",
                    controller: 'GameBox'
                }).when("/game/edit/:game_id", {
                    templateUrl: "views/pages/game-edit.html",
                    controller: 'GameEdit'
                }).when("/point-panel", {
            ...
            }]).factory("Player", ["$http", "$rootScope", function ($http, $rootScope) {
                var players = [];
                return {
            ...
            }]).factory("Coach", ["$http", "$rootScope", function ($http, $rootScope) {
                var coaches = [];
                return {
                    save: function (coach) {
            ...
            }]).factory("Team", ["$http", "$rootScope", function ($http, $rootScope) {
                var teams = [];
                return {
                    save: function (team) {
            ...
            }]).factory("Referee", ["$http", "$rootScope", function ($http, $rootScope) {
                var referees = [];
                return {
    ...
    angular.module("app.controllers", []).controller("AppCtrl", ["$scope", "$location", function ($scope, $location) {
    ...
    }]).controller("DashboardCtrl", ["$scope", function ($scope) {}]).controller("PlayerCtrl", ["$scope", "Player", "Team", function ($scope, Player, Team) {
        var is_new = false;
        $scope.player = {};
        ...
        $scope.$on('responseHandler', function (event, resp) {
            ...
            if (resp.method == 'team.get') $scope.teams = resp.data;
        });
        $scope.options = {
            genders: [{
                id: 'M',
        ...
        $scope.delete = function (player) {
            if (confirm('Delete ' + player.last_name + ' ?')) $scope.players.splice($scope.players.indexOf(player), 1);

        }
        $scope.save = function () {
            Player.save($scope.player);
        }
    }]).controller("CoachCtrl", ["$scope", "Coach", function ($scope, Coach) {
        var is_new = false;
        $scope.coach = {};
    ...
    }]).controller("RefereeCtrl", ["$scope", "Referee", function ($scope, Referee) {
        var is_new = false;
        $scope.referee = {};
    ...
    }]).controller("TeamCtrl", ["$scope", "Team", "Coach", "Player", function ($scope, Team, Coach, Player) {
        var is_new = false;
        $scope.teams = Team.get();
        $scope.coaches = Coach.get();
        $scope.players = Player.get();
    ...
    }]).controller("GameCreateCtrl", ["$scope", "$http", "Team", "Referee", "Game", function ($scope, $http, Team, Referee, Game) {
        Team.get();
        Referee.get();
            ...
            angular.forEach($scope.game.teams, function (team, index) {
                ...
                angular.forEach($scope.game.teams[index].players, function (player) {
                    playersArr.push({
                        ...
                        oncourt: player.starter,
                        position: player.position
                    });
                });
                $scope.game.boxscore.push({
                    team: {
                        _id: team._id,
    ...
    }]).controller("PlayByPlayCtrl", ["$scope", "socket", function ($scope, socket) {
        $scope.play_by_plays = [];
        ...
        var abs_plays = {
            game: {
                ...
                game_sequences: [{
                    ...
                    quarter: 0,
                    gametime: "",
                    school_1_running_score: 0,
                    school_2_running_score: 0
                }, ]
            }
        }
        $scope.$on('responseHandler', function (event, resp) {
...
$scope.saveGame();
    ...
    $scope.save_play = function(){      
        stats_minus($scope.selected_play);   
        //$scope.selected_play = $scope.updated_play;        
        $scope.selected_play.team_id = $scope.game.boxscore[$scope.updated_play.op.team_index].team._id;
        $scope.selected_play.team_name = $scope.game.boxscore[$scope.updated_play.op.team_index].team.team_name;
        $scope.selected_play.period = $scope.updated_play.period;
        $scope.selected_play.game_time = $scope.updated_play.game_time;
        if($scope.selected_play.op.player_index>-1){
            $scope.selected_play.player_id = $scope.game.boxscore[$scope.updated_play.op.team_index].players[$scope.updated_play.op.player_index]._id;
    ...
    var stats_minus =function(play){        
        alert(play.game_event)
           switch (play.game_event) {            
                ...
                break;
                case 'steal':
                $scope.game.boxscore[play.op.team_index].players[play.op.player_index].stl -= 1;
                $scope.game.boxscore[play.op.team_index].players[play.op.player_index].period[play.period - 1].stl -= 1;
                $scope.game.boxscore[play.op.team_index].team.stl -= 1;
                $scope.game.boxscore[play.op.team_index].team.period[play.period - 1].stl -= 1;;
                break;
                case 'turn_over':
                if(play.op.player_index>-1){
                    ...
                    $scope.game.boxscore[play.op.team_index].players[play.op.player_index].tov[play.op.type] -= 1;
                    $scope.game.boxscore[play.op.team_index].players[play.op.player_index].period[play.period - 1].tov[play.op.type] -= 1;
                } 
                ...
                //todo add to, adjust plusminus
                 if(play.op.wa){
                    $scope.game.boxscore[play.op.team_index].players[play.op.player_index].fou.wa -= 1;
                    $scope.game.boxscore[play.op.team_index].players[play.op.player_index].period[play.period - 1].fou.wa -= 1;
                    $scope.game.boxscore[play.op.team_index].team.fou.wa -= 1;
                    $scope.game.boxscore[play.op.team_index].team.period[play.period - 1].fou.wa -= 1;
                }
                $scope.game.boxscore[play.op.team_index].players[play.op.player_index].fou.tot -= 1;
                $scope.game.boxscore[play.op.team_index].players[play.op.player_index].period[play.period - 1].fou.tot -= 1;
                $scope.game.boxscore[play.op.team_index].team.fou.tot -= 1;
                $scope.game.boxscore[play.op.team_index].team.period[play.period - 1].fou.tot -= 1;
                ...
                if(play.op.made){
                    ...
                    $scope.game.boxscore[play.op.team_index].players[play.op.player_index].period[play.period-1].ft.md-=1;
                    $scope.game.boxscore[play.op.team_index].team.ft.md -= 1;
                ...
                case 'field_goal':
                if(play.op.made){
                    $scope.game.boxscore[play.op.team_index].players[play.op.player_index].pts -= play.op.value;
                ...
                if(play.op.value==2){
                    //2pts mades
                    if(play.op.made){
                        $scope.game.boxscore[play.op.team_index].players[play.op.player_index].fg2.md -= 1;
                        $scope.game.boxscore[play.op.team_index].players[play.op.player_index].period[play.period-1].fg2.md-=1;
                        $scope.game.boxscore[play.op.team_index].team.fg2.md -= 1;
                        $scope.game.boxscore[play.op.team_index].team.period[play.period-1].fg2.md -=1;
                        if(play.op.is_contested){
                            $scope.game.boxscore[play.op.team_index].players[play.op.player_index].fg2.contested.md -= 1;
                            $scope.game.boxscore[play.op.team_index].players[play.op.player_index].period[play.period-1].fg2.contested.md-=1;
                            $scope.game.boxscore[play.op.team_index].team.fg2.contested.md -= 1;
                            $scope.game.boxscore[play.op.team_index].team.period[play.period-1].fg2.contested.md -=1;
                        }
                    }
                    if(play.op.is_contested){
                        $scope.game.boxscore[play.op.team_index].players[play.op.player_index].fg2.contested.att -= 1;
                        $scope.game.boxscore[play.op.team_index].players[play.op.player_index].period[play.period-1].fg2.contested.att-=1;
                        $scope.game.boxscore[play.op.team_index].team.fg2.contested.att -= 1;
                        $scope.game.boxscore[play.op.team_index].team.period[play.period-1].fg2.contested.att -=1;
                    }
                           //2pts atts
                    $scope.game.boxscore[play.op.team_index].players[play.op.player_index].fg2.att -= 1;
                    $scope.game.boxscore[play.op.team_index].players[play.op.player_index].period[play.period-1].fg2.att-=1;
                    $scope.game.boxscore[play.op.team_index].team.fg2.att -= 1;
                    $scope.game.boxscore[play.op.team_index].team.period[play.period-1].fg2.att -=1;

                }else{
                    if(play.op.made){
                        $scope.game.boxscore[play.op.team_index].players[play.op.player_index].fg3.md -= 1;
                        $scope.game.boxscore[play.op.team_index].players[play.op.player_index].period[play.period-1].fg3.md-=1;
                        $scope.game.boxscore[play.op.team_index].team.fg3.md -= 1;
                        $scope.game.boxscore[play.op.team_index].team.period[play.period-1].fg3.md -=1;
                        if(play.op.is_contested){
                            $scope.game.boxscore[play.op.team_index].players[play.op.player_index].fg3.contested.md -= 1;
                            $scope.game.boxscore[play.op.team_index].players[play.op.player_index].period[play.period-1].fg3.contested.md-=1;
                            $scope.game.boxscore[play.op.team_index].team.fg3.contested.md -= 1;
                            $scope.game.boxscore[play.op.team_index].team.period[play.period-1].fg3.contested.md -=1;
                        }
                    }
                           //3pts atts
                    if(play.op.is_contested){
                        $scope.game.boxscore[play.op.team_index].players[play.op.player_index].fg3.contested.att -= 1;
                        $scope.game.boxscore[play.op.team_index].players[play.op.player_index].period[play.period-1].fg3.contested.att-=1;
                        $scope.game.boxscore[play.op.team_index].team.fg3.contested.att -= 1;
                        $scope.game.boxscore[play.op.team_index].team.period[play.period-1].fg3.contested.att -=1;
                    }
                    $scope.game.boxscore[play.op.team_index].players[play.op.player_index].fg3.att -= 1;
                    $scope.game.boxscore[play.op.team_index].players[play.op.player_index].period[play.period-1].fg3.att-=1;
                    $scope.game.boxscore[play.op.team_index].team.fg3.att -= 1;
                    $scope.game.boxscore[play.op.team_index].team.period[play.period-1].fg3.att -=1;

                }
                 //total fg mades
                if(play.op.made){
                    $scope.game.boxscore[play.op.team_index].players[play.op.player_index].fg.md -= 1;
                    $scope.game.boxscore[play.op.team_index].players[play.op.player_index].period[play.period-1].fg.md-=1;
                    $scope.game.boxscore[play.op.team_index].team.fg.md -= 1;
                    $scope.game.boxscore[play.op.team_index].team.period[play.period-1].fg.md -=1;
                }
                //if in paint
                if(play.op.is_in_paint){
                    if(play.op.made){
                        $scope.game.boxscore[play.op.team_index].players[play.op.player_index].fg.in_paint.md -= 1;
                        $scope.game.boxscore[play.op.team_index].players[play.op.player_index].period[play.period-1].fg.in_paint.md-=1;
                        $scope.game.boxscore[play.op.team_index].team.fg.in_paint.md -= 1;
                        $scope.game.boxscore[play.op.team_index].team.period[play.period-1].fg.in_paint.md -=1;
                        if(play.op.is_contested){
                            $scope.game.boxscore[play.op.team_index].players[play.op.player_index].fg.in_paint.contested.md -= 1;
                            $scope.game.boxscore[play.op.team_index].players[play.op.player_index].period[play.period-1].fg.in_paint.contested.md-=1;
                            $scope.game.boxscore[play.op.team_index].team.fg.in_paint.contested.md -= 1;
                            $scope.game.boxscore[play.op.team_index].team.period[play.period-1].fg.in_paint.contested.md -=1;
                        }
                    }
                    if(play.op.is_contested){
                        $scope.game.boxscore[play.op.team_index].players[play.op.player_index].fg.in_paint.contested.att -= 1;
                        $scope.game.boxscore[play.op.team_index].players[play.op.player_index].period[play.period-1].fg.in_paint.contested.att-=1;
                        $scope.game.boxscore[play.op.team_index].team.fg.in_paint.contested.att -= 1;
                        $scope.game.boxscore[play.op.team_index].team.period[play.period-1].fg.in_paint.contested.att -=1;
                    }
                    $scope.game.boxscore[play.op.team_index].players[play.op.player_index].fg.in_paint.att -= 1;
                    $scope.game.boxscore[play.op.team_index].players[play.op.player_index].period[play.period-1].fg.in_paint.att-=1;
                    $scope.game.boxscore[play.op.team_index].team.fg.in_paint.att -= 1;
                    $scope.game.boxscore[play.op.team_index].team.period[play.period-1].fg.in_paint.att -=1;
                }
                           //total fg atts
                if(play.op.is_contested){
                    if(play.op.made){
                        $scope.game.boxscore[play.op.team_index].players[play.op.player_index].fg.contested.md -= 1;
                        $scope.game.boxscore[play.op.team_index].players[play.op.player_index].period[play.period-1].fg.contested.md-=1;
                        $scope.game.boxscore[play.op.team_index].team.fg.contested.md -= 1;
                        $scope.game.boxscore[play.op.team_index].team.period[play.period-1].fg.contested.md -=1;    
                    }
                    $scope.game.boxscore[play.op.team_index].players[play.op.player_index].fg.contested.att -= 1;
                    $scope.game.boxscore[play.op.team_index].players[play.op.player_index].period[play.period-1].fg.contested.att-=1;
                    $scope.game.boxscore[play.op.team_index].team.fg.contested.att -= 1;
                    $scope.game.boxscore[play.op.team_index].team.period[play.period-1].fg.contested.att -=1;
                }
                $scope.game.boxscore[play.op.team_index].players[play.op.player_index].fg.att -= 1;
                $scope.game.boxscore[play.op.team_index].players[play.op.player_index].period[play.period-1].fg.att-=1;
                $scope.game.boxscore[play.op.team_index].team.fg.att -= 1;
                $scope.game.boxscore[play.op.team_index].team.period[play.period-1].fg.att -=1;
...
/*$scope.selected_player.periods[$scope.game.period-1].ass+=1;
        ...
        function saveFieldGoal() {
            if ($scope.field_goal.made) {
                ...
                $scope.game.boxscore[$scope.selected_team].team.pts+=$scope.field_goal.value;
            ...
            if ($scope.field_goal.value == 3) 
                ...
                desc = $scope.field_goal.made ? '2-Point Made' : '2-Point Missed';
            var action = {
        ...
        function save_play(action) {
            ...
            angular.forEach($scope.players_on_court[0],function(player){
                ...
                player_oc.player_no =player.no;
                player_oc.player_name =player.last_name+', '+player.first_name;
                player_oc.player_index=pid;
                players_on_court.push(player_oc);
            });
            ...
            action.op.players_on_court[1] = players_on_court;
        ...
        var clear = function () {
        ...
        }]).controller("GameBox", ["$scope", "$http", "Game", "$routeParams","$filter", function ($scope, $http, Game, $routeParams,$filter) {
            Game.get($routeParams.game_id);
            $scope.teams = [
            {
                team_name:'',
                starters:[],
            ...
            $scope.$on('responseHandler', function (event, resp) { 
             $scope.teams[0].team_name = resp.data.teams[0].team_name;
             $scope.teams[1].team_name = resp.data.teams[1].team_name;     
             $scope.teams[0].starters = $filter('filter')(resp.data.boxscore[0].players, {starter: true});
             $scope.teams[1].starters = $filter('filter')(resp.data.boxscore[1].players, {starter: true});
             $scope.teams[0].bench = $filter('filter')(resp.data.boxscore[0].players, {starter: false});
             $scope.teams[1].bench = $filter('filter')(resp.data.boxscore[1].players, {starter: false});
         });


        }]).controller("GameEdit", ["$scope", "$http", "Game", "$routeParams","$filter", function ($scope, $http, Game, $routeParams,$filter) {
            Game.get($routeParams.game_id);
            $scope.game={};
            $scope.teams = [
            {
            ...
            $scope.update_mins = function(player){
                player.mins=0;
                angular.forEach(player.period,function(period){
                    player.mins+=period.mins;
                });

            }
            $scope.update_sc_pts = function(team){
                $scope.game.boxscore[team].team.sc_pts = 0;
                angular.forEach($scope.game.boxscore[team].team.period,function(period){
                   $scope.game.boxscore[team].team.sc_pts += period.sc_pts;
                });

            }
            $scope.update_fb_pts = function(team){
                $scope.game.boxscore[team].team.fb_pts.pts = 0;
                angular.forEach($scope.game.boxscore[team].team.period,function(period){
                   $scope.game.boxscore[team].team.fb_pts.pts += period.fb_pts.pts;
                });

            }

            $scope.update_fb_att = function(team){
                $scope.game.boxscore[team].team.fb_pts.att = 0;
                angular.forEach($scope.game.boxscore[team].team.period,function(period){
                   $scope.game.boxscore[team].team.fb_pts.att += period.fb_pts.att;
                });
            ...
            $scope.$on('responseHandler', function (event, resp) { 
                ...
                $scope.init_starters();
                for(i=1;i< $scope.game.period+1;i++){
                    if(i<5)
                        $scope.periods.push('Q'+i)
                    else
                        $scope.periods.push('OT'+(i-4))
                 }
                }
                if(resp.method=='game.save'){
                    alert('Game Saved');
            ...
            $scope.init_starters = function(){
             $scope.teams[0].team_name =  $scope.game.teams[0].team_name;
             $scope.teams[1].team_name =  $scope.game.teams[1].team_name;     
             $scope.teams[0].starters = $filter('filter')( $scope.game.boxscore[0].players, {starter: true});
             $scope.teams[1].starters = $filter('filter')( $scope.game.boxscore[1].players, {starter: true});
             $scope.teams[0].bench = $filter('filter')( $scope.game.boxscore[0].players, {starter: false});
             $scope.teams[1].bench = $filter('filter')( $scope.game.boxscore[1].players, {starter: false});
            }
