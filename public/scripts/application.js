...
function () {
	...
	function () {
		...
		function () {
			angular.module("app", ["ngRoute", "ngAnimate", "ui.bootstrap", "easypiechart", "mgo-angular-wizard", "textAngular", "app.ui.ctrls", "app.ui.directives", "app.ui.services", "app.controllers", "app.directives", "app.form.validation", "app.ui.form.ctrls", "app.ui.form.directives", "app.tables", "app.task", "app.localization", "app.chart.ctrls", "app.chart.directives","timer","socket-io"]).config(["$routeProvider",
	...
	function () {
		...
		function () {
			...
			angular.module("app.controllers", []).controller("AppCtrl", ["$scope", "$location",
				...
				]).controller("GameCreateCtrl", ["$scope","$http",
				function ($scope,$http) {

					$scope.options = {
						teams: [
							{team_id:1,name:'NU',mascott:'Bulldogs',coach:{name:'Altramirano',id:1},assistants:[{name:'Tsupeng',id:2}],players:[
								{pid:1,pfn:'John',pln:'Doe',pno:01,strt:1,position:'G'},
								{pid:2,pfn:'Lisa',pln:'Doe',pno:02,strt:1,position:'PG'},
								{pid:3,pfn:'GG',pln:'Kanor',pno:03,strt:1,position:'SG'},
								{pid:4,pfn:'MAni',pln:'Pak',pno:04,strt:1,position:'C'},
								{pid:5,pfn:'Chup',pln:'Chupa',pno:10,strt:1,position:'F'},
							]},
							{team_id:2,name:'UST',mascott:'Growling Tigers',coach:{name:'Pido Jarencio',id:2},assistants:[{name:'Tsupeng',id:2}],players:[
								{pid:1,pfn:'Vhilly',pln:'Santiago',pno:11,strt:1,position:'G'},
								{pid:2,pfn:'Enteng',pln:'Arica',pno:12,strt:1,position:'PG'},
								{pid:3,pfn:'Joey',pln:'Cabug',pno:13,strt:1,position:'SG'},
								{pid:4,pfn:'Dandy',pln:'Collera',pno:14,strt:1,position:'C'},
								{pid:5,pfn:'Pong',pln:'Ducanes',pno:11,strt:1,position:'F'},
							]},
						],
						officials:[
							{id:1,name:'Soriano'},
					...
					$scope.save = function(){
						angular.forEach($scope.game.boxscore,function(team,teamcolor){               
							angular.forEach($scope.game.team[teamcolor].players,function(player){								
								$scope.game.boxscore[teamcolor].players.push({pid:player.pid,pfn:player.pfn,pln:player.pln,pno:player.pno,strt:player.strt,position:player.position,mins:0,ass:0,tov:{tot:0,travelling:0,double_dribble:0,bad_pass:0,three_sec:0,five_sec:0,eight_sec:0,shot_clock:0,off_foul:0,ball_handling:0},stl:0,pts:0,eff:0,srt:1,fg:{md:0,att:0,pct:0.0},fg2:{md:0,att:0,pct:0.0},fg3:{md:0,att:0,pct:0.0},ft:{md:0,att:0,pct:0.0},reb:{off:0,def:0,tot:0},blk:{bl:0,bo:0},fou:{pf:0,fo:0}});								
							});                		
						});    
						$http.post('/api/game', $scope.game).success(function(data) {
				...
				]).controller("PointPanelCtrl", ["$scope", "$http","socket",
                ...
                {id:'D',name:'Dunk'},
                 ...
                 $http.get('/api/game/53b76bb748d299701218e1b1').success(function(data) {
                 	$scope.game = data;
                 }).error(function(data) {
                ...
                $scope.player_select = function (team, player) {
                	...
                	switch ($scope.current_action) {
                		...
                		break;
                		case 'rebound':
                		saveRebound(team.team_id == $scope.selected_team.team_id);
                		clear();
                		break;
                		case 'block':
                		saveBlock();
                ...
                //shot attempt action
                $scope.shot_attempt = function (value, zone) {
                	if(!ready())
                		return;
                	$scope.selected_player = '';
                ...
                function saveRebound(isOffensive) {
                	if(isOffensive){
                		$scope.selected_player.reb.off += 1;
                		$scope.selected_player.periods[$scope.game.period-1].reb.off+=1;
                	}else{
                		$scope.selected_player.reb.def += 1;
                		$scope.selected_player.periods[$scope.game.period-1].reb.def+=1;
                	}
                	$scope.selected_player.reb.tot += 1;
                	$scope.selected_player.periods[$scope.game.period-1].reb.tot+=1;
                	$scope.selected_player = '';
                	$scope.current_action = '';
                	saveGame();
                };
                ...
                function saveTurnOver() {
                	...
                	alert($scope.selected_player.tov.tot);
                	saveGame();
                };

                function saveBlock() {
                	$scope.selected_player.blk.bl += 1;
                	$scope.selected_player.periods[$scope.game.period-1].blk.bl+=1;
                	saveGame();
                };
                ...
                function saveFieldGoal() {
                	...
                	if ($scope.field_goal.made) {
                		...
                		$scope.selected_team.score += $scope.field_goal.value;

                		$scope.selected_player.periods[$scope.game.period-1].pts += $scope.field_goal.value;
						//$scope.selected_team.score += $field_goal.value;

						if($scope.field_goal.value==1){

							$scope.selected_player.ft.md += 1;
							$scope.selected_player.periods[$scope.game.period-1].ft.md += 1;

						}else{
							if($scope.field_goal.value==2){

								$scope.selected_player.fg2.md+=1;
								$scope.selected_player.periods[$scope.game.period-1].fg2.md+=1;

							}else{

								$scope.selected_player.fg3.md+=1;
								$scope.selected_player.periods[$scope.game.period-1].fg3.md+=1;


							}

							$scope.selected_player.fg.md+=1;
							$scope.selected_player.periods[$scope.game.period-1].fg.md+=1;
						...
						if($scope.field_goal.blocked){  
							...
							$scope.selected_player.periods[$scope.game.period-1].blk.bo+=1;         			
					...
					if($scope.field_goal.value==1){
						...
						$scope.selected_player.periods[$scope.game.period-1].ft.att+=1;

					}else{ 						
						if($scope.field_goal.value==3){
							shot_type = "3pt";
							$scope.selected_player.fg3.att+=1;
							$scope.selected_player.periods[$scope.game.period-1].fg3.att+=1;

						}else{
							shot_type = $scope.field_goal.shot_type.name;	
							$scope.selected_player.fg2.att+=1;
							$scope.selected_player.periods[$scope.game.period-1].fg2.att+=1;

						}
						$scope.selected_player.fg.att+=1;
					}
					play = $scope.selected_team.team_name+' '+$scope.selected_player.pln+' '+shot_type+' shot:'+result;
				...
				function save_play(team_name,team_id,player_id,player_name,player_no,game_clock,game_period,desc){


					$scope.play_by_play.push({
						team_name:team_name,
						team_id:team_id,
						player_id:player_id,
				...
				$scope.startGame = function(){
					...
					angular.forEach($scope.game.boxscore,function(team){               
						angular.forEach(team.players_in_game,function(player){

							team.players.push
							player.periods.push({no:period,mins:0,ass:0,tov:{tot:0,travelling:0,double_dribble:0,bad_pass:0,three_sec:0,five_sec:0,eight_sec:0,shot_clock:0,off_foul:0,ball_handling:0},stl:0,pts:0,eff:0,srt:1,fg:{md:0,att:0,pct:0.0},fg2:{md:0,att:0,pct:0.0},fg3:{md:0,att:0,pct:0.0},ft:{md:0,att:0,pct:0.0},reb:{off:0,def:0,tot:0},blk:{bl:0,bo:0},fou:{pf:0,fo:0}});
						});                		
					});    
					$scope.game.period += 1;
                ...
                var saveGame = function(){                
                	$http.post('/api/game/update', game)
                ...
                var player_select_btn_switch = function (enabled, team) {
                	if (team) {
                		if ($scope.selected_team.team_id == $scope.team['light'].team_id) {
